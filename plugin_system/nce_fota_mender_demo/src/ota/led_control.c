...
LOG_MODULE_REGISTER(led_control, 3);
...
static void led_pattern_success(int frame_number)
{
	if (IS_ENABLED(CONFIG_LED_INDICATOR_RGB)) {
		...
		#if ( CONFIG_APPLICATION_VERSION == 1 )
    	/* V1 - Green */
		led_set_rgb(0, 100 , 0);
    	#else
    	/* V2 - Blue */
		led_set_rgb(0, 0 , 100);
		#endif /* if ( CONFIG_APPLICATION_VERSION == 1 ) */
...
/**
 * @brief Change the LEDs to a pattern that indicates connecting.
 *
 * @param frame_number - An integer used for basic animations.
 */
static void led_pattern_connecting(int frame_number)
{
	...
	if (IS_ENABLED(CONFIG_LED_INDICATOR_RGB)) {
		/* Pulsating white  */
		led_set_rgb(breath * 10, breath * 10, breath * 10);
	}
}
...
static void led_pattern_waiting(int frame_number)
{
	if (IS_ENABLED(CONFIG_LED_INDICATOR_RGB)) {
		...
		#if ( CONFIG_APPLICATION_VERSION == 1 )
    	/* V1 - Blinking Green */
		led_set_rgb(0, breath * 10, 0);
    	#else
    	/* V2 - Blinking Blue */
		led_set_rgb(0,0, breath * 10);
		#endif /* if ( CONFIG_APPLICATION_VERSION == 1 ) */
...
static void animate_leds_frame(int frame_number, enum led_pattern pattern)
{
	...
	case LED_IDLE:
		...
		break;
	case LED_CONNECTING:
		led_pattern_connecting(frame_number);
		break;
	case LED_WAITING:
