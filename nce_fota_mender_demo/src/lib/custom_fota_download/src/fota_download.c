...
#define FILE_BUF_LEN ((CONFIG_CUSTOM_DOWNLOAD_CLIENT_MAX_FILENAME_SIZE*2)+1)
#else
#define FILE_BUF_LEN (CONFIG_CUSTOM_DOWNLOAD_CLIENT_MAX_FILENAME_SIZE)
#endif

LOG_MODULE_REGISTER(fota_download, CONFIG_CUSTOM_FOTA_DOWNLOAD_LOG_LEVEL);
...
static uint8_t mcuboot_buf[CONFIG_CUSTOM_FOTA_DOWNLOAD_MCUBOOT_FLASH_BUF_SZ] __aligned(4);
#endif
...
{
	__ASSERT(id != CUSTOM_FOTA_DOWNLOAD_EVT_PROGRESS, "use send_progress");
	__ASSERT(id != CUSTOM_FOTA_DOWNLOAD_EVT_ERROR, "use send_error_evt");
...
{
	__ASSERT(cause != CUSTOM_FOTA_DOWNLOAD_ERROR_CAUSE_NO_ERROR, "use a valid error cause");
	const struct fota_download_evt evt = {
		.id = CUSTOM_FOTA_DOWNLOAD_EVT_ERROR,
		.cause = cause
	};
	first_fragment = true;
	downloading = false;
...
{
#ifdef CONFIG_CUSTOM_FOTA_DOWNLOAD_PROGRESS_EVT
	const struct fota_download_evt evt = { .id = CUSTOM_FOTA_DOWNLOAD_EVT_PROGRESS,
					       .progress = progress };
	callback(&evt);
#endif
}
...
static void dfu_target_callback_handler(enum dfu_target_evt_id evt)
{
	...
	case DFU_TARGET_EVT_TIMEOUT:
		send_evt(CUSTOM_FOTA_DOWNLOAD_EVT_ERASE_PENDING);
		break;
	case DFU_TARGET_EVT_ERASE_DONE:
		send_evt(CUSTOM_FOTA_DOWNLOAD_EVT_ERASE_DONE);
		break;
	default:
		send_error_evt(CUSTOM_FOTA_DOWNLOAD_ERROR_CAUSE_DOWNLOAD_FAILED);
	}
}
...
static int download_client_callback(const struct download_client_evt *event)
{
	...
	switch (event->id) {
	case CUSTOM_DOWNLOAD_CLIENT_EVT_FRAGMENT: {
		if (first_fragment) {
			...
			if (err != 0) {
				LOG_DBG("download_client_file_size_get err: %d",
					err);
				send_error_evt(CUSTOM_FOTA_DOWNLOAD_ERROR_CAUSE_INTERNAL);
				return err;
			}
			...
			if (img_type == DFU_TARGET_IMAGE_TYPE_NONE) {
				...
				err_cause = CUSTOM_FOTA_DOWNLOAD_ERROR_CAUSE_INVALID_UPDATE;
				err = -EFAULT;
				...
				err_cause = CUSTOM_FOTA_DOWNLOAD_ERROR_CAUSE_TYPE_MISMATCH;
				err = -EPROTOTYPE;
			} else {
				...
				if (err == -EFBIG) {
					...
					err_cause = CUSTOM_FOTA_DOWNLOAD_ERROR_CAUSE_INTERNAL;
				}
			}

			if (err_cause != CUSTOM_FOTA_DOWNLOAD_ERROR_CAUSE_NO_ERROR) {
		...
		if (err != 0) {
			...
			first_fragment = true;
			(void) download_client_disconnect(&dlc);
			send_error_evt(CUSTOM_FOTA_DOWNLOAD_ERROR_CAUSE_INVALID_UPDATE);
			return err;
		}

		if (IS_ENABLED(CONFIG_CUSTOM_FOTA_DOWNLOAD_PROGRESS_EVT) &&
		    !first_fragment) {
			err = dfu_target_offset_get(&offset);
			if (err != 0) {
	...
	case CUSTOM_DOWNLOAD_CLIENT_EVT_DONE:
		err = dfu_target_done(true);
		if (err == 0 && IS_ENABLED(CONFIG_CUSTOM_FOTA_CLIENT_AUTOSCHEDULE_UPDATE)) {
		...
		send_evt(CUSTOM_FOTA_DOWNLOAD_EVT_FINISHED);
		first_fragment = true;
		downloading = false;
		break;

	case CUSTOM_DOWNLOAD_CLIENT_EVT_ERROR: {
	...
	const enum dfu_target_image_type expected_type)
{
	...
	socket_retries_left = CONFIG_CUSTOM_FOTA_SOCKET_RETRIES;
...
#ifdef CONFIG_CUSTOM_FOTA_DOWNLOAD_NATIVE_TLS
	/* Enable native TLS for the download client socket
	 * if configured.
	 */
	dlc.set_native_tls = CONFIG_CUSTOM_FOTA_DOWNLOAD_NATIVE_TLS;
#endif
...
int fota_download_cancel(void)
{
	...
	if (err && err != -EACCES) {
		...
		first_fragment = true;
		send_evt(CUSTOM_FOTA_DOWNLOAD_EVT_CANCELLED);
	}
