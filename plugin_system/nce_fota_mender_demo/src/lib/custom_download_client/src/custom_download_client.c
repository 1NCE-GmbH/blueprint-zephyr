...
LOG_MODULE_REGISTER(download_client, CONFIG_CUSTOM_DOWNLOAD_CLIENT_LOG_LEVEL);

#define SIN6(A) ((struct sockaddr_in6 *)(A))
#define SIN(A) ((struct sockaddr_in *)(A))

#define HOSTNAME_SIZE CONFIG_CUSTOM_DOWNLOAD_CLIENT_MAX_HOSTNAME_SIZE
...
#define MAGIC_HEADER_RANGE 65536
uint32_t magic_header[] = {0x96, 0xf3, 0xb8, 0x3d};
...
{
	__ASSERT(client->offset <= CONFIG_CUSTOM_DOWNLOAD_CLIENT_BUF_SIZE,
		 "Buffer overflow!");

	const struct download_client_evt evt = {
		.id = CUSTOM_DOWNLOAD_CLIENT_EVT_FRAGMENT,
		.fragment = {
			.buf = client->buf,
			.len = client->offset,
		}
...
static int error_evt_send(const struct download_client *dl, int error)
{
	...
	const struct download_client_evt evt = {
		.id = CUSTOM_DOWNLOAD_CLIENT_EVT_ERROR,
		.error = -error
	};
...
static ssize_t socket_recv(struct download_client *dl)
{
	...
	case IPPROTO_TCP:
	case IPPROTO_TLS_1_2:
		timeout = CONFIG_CUSTOM_DOWNLOAD_CLIENT_TCP_SOCK_TIMEO_MS;
		break;
	case IPPROTO_UDP:
	case IPPROTO_DTLS_1_2:
		if (IS_ENABLED(CONFIG_COAP)) {
...
void download_thread(void *client, void *a, void *b)
{
	...
	size_t max_size = secondary_size[0];
...
wait_for_download:
	...
	while (dl->fd != -1) {
		...
		if( resume == 0)
		{
		for(int i=0;i<(sizeof(dl->buf));i++){
			
			exist =  memcmp ( dl->buf + i, magic_header, 4);

			if( (exist == 0 || dl->progress > MAGIC_HEADER_RANGE) && resume == 0)
			{
				resume =1;
		...
		if (dl->progress == dl->file_size) {
			...
			const struct download_client_evt evt = {
				.id = CUSTOM_DOWNLOAD_CLIENT_EVT_DONE,
			};
			dl->callback(&evt);
			/* Restart and suspend */
			break;
...
send_again:
		...
		if (dl->proto != IPPROTO_TCP || len == 0
		   || IS_ENABLED(CONFIG_CUSTOM_DOWNLOAD_CLIENT_RANGE_REQUESTS)) {
			    ...
			    const struct download_client_cfg *config)
{
	...
	if (config->frag_size_override > CONFIG_CUSTOM_DOWNLOAD_CLIENT_BUF_SIZE) {
	...
	if (IS_ENABLED(CONFIG_CUSTOM_DOWNLOAD_CLIENT_IPV6)) {
		err = host_lookup(host, AF_INET6, config->pdn_id, &client->remote_addr);
	}
	if (err || !IS_ENABLED(CONFIG_CUSTOM_DOWNLOAD_CLIENT_IPV6)) {
